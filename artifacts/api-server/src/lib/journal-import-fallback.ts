...
import { resolveJournalFeaturedImageUrlWithDefault, rewriteJournalContentHtml } from "./journal-image-url";
...
export function journalRowsToDtos(rows: JournalImportRow[]): JournalPostDto[] {
  return rows.map((row, index) => ({
    ...
    featuredImageUrl: resolveJournalFeaturedImageUrlWithDefault(row.featuredImageUrl),
    author: row.author,
    categoryId: null,
